...
const getCoachSeatMap = async (req, res) => {
    try {
        ...
        const route = booking.trainId?.route || [];

        const getStationIndex = (stationCode) =>
            route.findIndex(
                (station) => station.stationCode === stationCode
            );

        const requestedStart =
            getStationIndex(booking.fromStation);

        const requestedEnd =
            getStationIndex(booking.toStation);
        ...
        const safeCoaches = coaches.map((coach) => {
            ...
            const seats = coach.seats.map((seat) => {
                ...
                if (
                    ...
                    const bookedStart =
                        getStationIndex(seat.fromStation);

                    const bookedEnd =
                        getStationIndex(seat.toStation);
                ...
                return {
                    ...
                    column: seat.column,
                    side: seat.side,
                    fromStation: seat.fromStation,
                    toStation: seat.toStation,
                    status: seatStatus,
